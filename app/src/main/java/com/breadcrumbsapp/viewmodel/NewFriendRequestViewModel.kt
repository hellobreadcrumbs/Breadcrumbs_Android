...
class NewFriendRequestViewModel : ViewModel() {
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                if (response.isSuccessful) {
                    ...
                    if (response.body()!!.status) {
                        ...
                        println("List== receivedList ${receivedList.size}")
                        println("List== sentList ${sentList.size}")
    ...
    fun accepORCanceltFriendRequest( id : String, status : Boolean ){
        ...
        CoroutineScope(Dispatchers.IO).launch {
            val response = apiService.acceptORCancelFriendRequest(
                mContext?.resources?.getString(R.string.api_access_token)?:"",
                requestBody
            )
            if (response.isSuccessful) {
                ...
                if (response.code() == 200){
                    requestStatus.postValue( id.toInt())
                }else{
